...
- At **authentication** you get `clientExtensionResults.prf.results.first` as an `ArrayBuffer`. If this is `undefined`, authentication still succeeded from the server's perspective, but you can't derive the key — you must tell the user their passkey provider doesn't support PRF and block the sign-in.
